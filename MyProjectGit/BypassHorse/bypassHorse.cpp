...
int choosingMove		// функция выбора хода
(
...
bool nextMove		// функция выполнения следующего хода
(
...
{
	bool defect[9]{};			// массив забракованных ходов из принятых (true - )

	int numMove = 0;		// номер выбранного хода

	do
	{
		numMove = choosingMove(moveNumber, path, chessboard, defect);	// номер выбранного хода 

		if (numMove)								// если номер выбранного хода не равен 0
		{
			if (moveNumber < 64)					// если номер хода меньше предельного (с учетом +1 далее)
			{
				...
				if (nextMove(moveNumber, path, chessboard)) // если результат вызова функции true
				{
...
//					chessboard[path[moveNumber][0]][path[moveNumber][1]] = 0;
			...
			else	// иначе, если номер хода равен или больше предельного - доска вся пройдена
			{
...
int main()
{
	...
	int path[65][2]{};				// путь коня по шахматной доске
	...
	cout
		...
		N N"     Принимаем, что путь коня не замкнут."
	...
	(void)nextMove(1, path, chessboard);		// функция выполнения ходов
