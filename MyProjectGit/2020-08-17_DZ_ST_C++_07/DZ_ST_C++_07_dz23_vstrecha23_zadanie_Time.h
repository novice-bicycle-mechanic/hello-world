...
namespace spaceTime
{
	...
	class Test : public Time {
	public:
		Test() {}

		Test(size_t hour_, size_t minute_ = 0, size_t second_ = 0) : Time(hour_, minute_, second_) {}

		// прототип дружественной функции перегрузки оператора +
		friend Test operator+(const Test& time1, const Test& time2);
	};
