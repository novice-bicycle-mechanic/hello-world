...
namespace spaceFraction
{
	...
	class Test : public Fraction {
	public:
		Test() {}

		Test(int numerator_, int denominator_) : Fraction(numerator_, denominator_) {}

		// прототип дружественной функции перегрузки оператора +
		friend Test operator+(const Test& fraction1, const Test& fraction2);
	};
